...
class Controller():
    ...
    def converting_fl(self, list_path: list) ->None: 
        ...
        for i, path in enumerate(list_path,1):
            ...
            self.create_tiff_files(dt, path) # запуск функции конвертация
            print(f'Конвертация файла завершена, создан следующий файл: out_{dt}.tiff')
            print("--------------------------------------------------------------------------------------")
        print("\nКонвертация завершена, после закрытия логирование будет удалено.") 
        self.startfine()
    
    def create_tiff_files(self,dt: str, path: str) -> None:        
        ...
        for i in range(len(images)):
            images[0].save(os.path.join(PATH_FILE_OUT, f"out_{dt}.tiff"),save_all=True,append_images=images[1:],compression='tiff_lzw') 
            progress_bar.update() 
        progress_bar.close()
